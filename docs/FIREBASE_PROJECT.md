...
The Windows beta development package on this machine is `release/weekdeck-beta/win-unpacked/Weekdeck.exe`. Native release builds run in the [Release Beta workflow](https://github.com/Voyager162/WeekDeck/actions/workflows/release.yml), including desktop startup checks, Android APK/AAB and lint, and an unsigned iOS archive on Xcode 26+. Check the successful run associated with each release before distributing artifacts. Production signing and physical iOS/Android notification checks remain outstanding. See [mobile reminder behavior](NOTIFICATIONS.md) and [store submission](STORE_SUBMISSION.md).
